package com.xaarlox.pw01.task01.ui.theme
...
fun EcoGridTheme(
    ...
    // Kept as a parameter for flexibility, but defaults to false: dynamic (wallpaper-based) colors would override our intentional warm palette
...
/**
 * Returns a progressively darker background color depending on the screen's
 * difficulty level (1 = lightest, 3 = darkest), matching either the light
 * or dark theme automatically
 */
@Composable
fun screenBackgroundForLevel(level: Int): Color {
    val dark = isSystemInDarkTheme()

    return when (level) {
        1 -> if (dark) ScreenBgLevel1Dark else ScreenBgLevel1Light
        2 -> if (dark) ScreenBgLevel2Dark else ScreenBgLevel2Light
        else -> if (dark) ScreenBgLevel3Dark else ScreenBgLevel3Light
    }
}